...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, task_num=1):
        ...
        self.bn1 = nn.ModuleList([nn.BatchNorm2d(mid_channels) for i in range(task_num)])
        self.bn2 = nn.ModuleList([nn.BatchNorm2d(out_channels) for i in range(task_num)])
        self.conv1 = nn.ModuleList([nn.Conv2d(in_channels,
                                              mid_channels,
                                              kernel_size=3,
                                              padding=1) for i in range(task_num)])
        self.conv2 = nn.ModuleList([nn.Conv2d(mid_channels,
                                              out_channels,
                                              kernel_size=3,
                                              padding=1) for i in range(task_num)])
...
class TriUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if not isinstance(in__channels, list):
        ...
        for i, (n_chan, n_class) in enumerate(zip(in_channels, out_channels)):
            setattr(self, 'in{}'.format(i=i), OutConv(n_chan, 64))
            setattr(self, 'out{}'.format(i=i), OutConv(64, n_class))
